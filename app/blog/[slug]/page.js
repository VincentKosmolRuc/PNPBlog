...
export default async function BlogPostPage({ params }) {
  ...
  <div className="max-w-4xl mx-auto space-y-2">
    {posts.map(post=> (
        <div key={post.id}>
            <h1  className="pt-6 font-extrabold text-center text-4xl text-gray-700">{post.title}</h1>
            <Image
                    src={`/${post.front_image}`}
                    width={1920}
                    height={1080}
                    alt={post.title}
                    className="aspect-[16/9] object-cover w-full pt-4 pb-1"
            />
            <p className="">{format(post.created_at, 'LLLL d, yyyy')}</p>
        </div>
        ))}
